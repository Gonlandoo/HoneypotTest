...
def initGlobalVars():
    ...
    results = {
        ...
        "inheritance_disorder": False, "type_deduction_overflow": False, "skip_empty_string_literal": False,
...
def change_format():
    with open(c_name) as disasm_file:
        file_contents = disasm_file.readlines()  # 读取所有行然后把它们作为一个字符串列表返回
        i = 0
        ...
        for line in file_contents:
            ...
            try:  # removing initial zeroes
                ...
                if(i==0):
                    ...
                    lineParts[0]=lineParts[0].lstrip('0')
            except:
...
def  full_sym_exec():
    ...
    # analysis=init_analysis()
    # params=Parameter(path_conditions_and_vars=path_conditions_and_vars,analysis=analysis)
...
def collect_vertices():
    ...
    with open(file, 'r') as disasm_file:
        text_lines = disasm_file.readlines()
        for current_line_content in text_lines:
            line_arr = current_line_content.split(" ")
            current_ins_address = int(line_arr[0], 16)
            current_ins = line_arr[1].strip("\n")
            if is_new_block:
                current_block = current_ins_address
                is_new_block = False
            if len(line_arr) > 2:
                current_data = current_line_content.split(" ")[2].strip("\n")
                instructions[current_ins_address] = current_ins + ' ' + current_data
            elif len(line_arr) <= 2:
                instructions[current_ins_address] = current_ins.strip("\n")
            if current_ins == "JUMPDEST":
                if last_ins_address not in end_ins_dict:
                    end_ins_dict[current_block] = last_ins_address
                current_block = current_ins_address
                is_new_block = False
            elif current_ins == "STOP" or current_ins == "RETURN" or current_ins == "SUICIDE" or current_ins == "REVERT" or current_ins == "ASSERTFAIL":
                jump_type[current_block] = "terminal"
                end_ins_dict[current_block] = current_ins_address
            elif current_ins == "JUMP":
                jump_type[current_block] = "unconditional"
                end_ins_dict[current_block] = current_ins_address
                is_new_block = True
            elif current_ins == "JUMPI":
                jump_type[current_block] = "conditional"
                end_ins_dict[current_block] = current_ins_address
                is_new_block = True
            elif current_ins.startswith('PUSH', 0):
                wait_for_push = True
            ...
            if isAllReal(s0, s1):
                ...
                if input in sha3_list:
                    ...
                    if symbolic:
                        ...
                        hash= hashlib.sha3_256(input).hexdigest()
    ...
    #
        ...
        if len(stack) > 1:
            ...
            if isReal(stored_address):
                ...
                old_size = len(memory) // 32#整除
                new_size = ceil32(stored_address + 32) // 32#向上取整
...
if __name__ == '__main__':
    file = 'honeypots\For_test.txt'
    # file='D:\K\sc\Graduate\Test.txt'
    main(file,None)
